...
class BotZangado(Bot):
    def __init__(self, nome):
        ...
        self.__comandos = [Comando('Bom dia', 'Bom dia pra QUEM? QUEM TA TENDO UM BOM DIA?\nQueria estar aqui nao po.'),
                           Comando(
                               'Quero conselho', 'Time que está ganhando não se mexe. Você estava ganhando antes de vir conversar comigo.'),
                           Comando(
                               'Por que zangado?', 'MEUS AMIGOS SÓ MARCAM O FUT AS 22H DA NOITE DESGRAÇA...COMO QUE EU SAIO DE CASA AS 22H NA CHUVA?????COMO????')
                           ]
